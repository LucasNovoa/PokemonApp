...
export function postPokemon(payload){
    return async function(dispatch){
        const dbPokemon = await axios.post('http://localhost:3001/pokemons', payload);
        return dbPokemon;
    }
}
...
export function getPokemonBySearchId(searchId){
    return async function(dispatch){
        try{
            ...
            // alert('There is no Pokemon with that ID. Try using from 1 to 898 or 10.001 to 10.228!')
            // console.log(error);
        }
    }
}
